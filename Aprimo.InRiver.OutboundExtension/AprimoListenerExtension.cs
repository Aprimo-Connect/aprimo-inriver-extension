...
namespace Aprimo.InRiver.OutboundExtension
{
    public class InRiverAprimoListener : IEntityListener, ILinkListener
    {
        ...
        public void EntityUpdated(int entityId, string[] fields)
        {
            ...
            Context.Log(inRiver.Remoting.Log.LogLevel.Debug, $"Aprimo Environment- Entity {entityId.ToString()} updated with fields {fields}");
            ...
            entityLinks.ForEach(delegate (Link item)
            {
                ...
                if((bool)currEntity.GetField(Context.Settings["aprimoResourceFromAprimoFieldTypeID"])?.Data == true)
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Context.Log(inRiver.Remoting.Log.LogLevel.Debug, "Aprimo Environment- Created Connector State");
                    }
                }
            });
        }
        ...
        {
            Context.Log(inRiver.Remoting.Log.LogLevel.Debug, "Aprimo Environment- Link Created() Called");
